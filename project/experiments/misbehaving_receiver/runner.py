...
parser = argparse.ArgumentParser(
    description="Create a network topology and test clients and servers."
)
parser.add_argument(
    "--delay", type=float, help="Link propagation delay (ms)", default=50
)
parser.add_argument(
    "--bandwidth", type=float, help="Bandwidth of network links (Mb/s)", default=1000
)
parser.add_argument(
    "--queuesize",
    type=int,
...
parser.add_argument(
    "--client",
    type=str,
    choices=["kernel", "opt-ack", "ack-division", "dup-ack"],
    help="The client type to use.",
    default="kernel",
)
parser.add_argument(
    "--server",
    type=str,
    choices=["kernel", "lwip", "lwip-defended"],
...
parser.add_argument(
    "--cong-control",
    type=str,
    choices=["reno", "cubic", "vegas"],
    help="Chosen TCP Congestion Control algorithm if the Linux kernel TCP server type is used.",
    default="reno",
)
parser.add_argument(
    "--cli",
    help="Instead of running the server and client, open a Mininet CLI to run manual Mininet commands.",
    action="store_true",
)
...
if __name__ == "__main__":
    ...
    if not args.cli:
        tcpdump = net.get("client").popen(
            "tcpdump -w captures/%s-%s.pcap" % (args.client, args.server)
        )

        if args.server == "kernel":
            os.system(
                "sysctl -w net.ipv4.tcp_congestion_control=%s" % args.cong_control
            )
        ...
        if args.client == "kernel":
            ...
            client = client_node.popen(
                "python attackers/opt_ack_attacker.py --host %s --dport %d"
                % (server_ip, port)
            )
        elif args.client == "ack-division":
            client = client_node.popen(
                "python attackers/ack_division_attacker.py --host %s --dport %d"
                % (server_ip, port)
            )
        elif args.client == "dup-ack":
            client = client_node.popen(
                "python attackers/dup_ack_attacker.py --host %s --dport %d"
                % (server_ip, port)
            )
        else:
