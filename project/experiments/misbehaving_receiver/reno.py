...
DUMMY_PAYLOAD = "*" * MSS
# Normal sender (server)
H1_ADDR = "10.0.0.1"
H1_PORT = 20001
# Attacker (misbehaving receiver)
H2_ADDR = "10.0.0.2"
H2_PORT = 20002
# Other normal receivers
H3_ADDR = "10.0.0.3"
H4_ADDR = "10.0.0.4"
H5_ADDR = "10.0.0.5"
H6_ADDR = "10.0.0.6"
...
class TCP_Client:
    def __init__(self, role, host, **kwargs):
        ...
        if host == "h1":
            ...
            self.src_ip = H3_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H3_PORT
            self.dst_port = H1_PORT

        elif host == "h4":
            self.src_ip = H4_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H4_PORT
            self.dst_port = H1_PORT

        elif host == "h5":
            self.src_ip = H5_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H5_PORT
            self.dst_port = H1_PORT

        elif host == "h6":
            self.src_ip = H6_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H6_PORT
            self.dst_port = H1_PORT

        elif host == "h7":
            self.src_ip = H7_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H7_PORT
            self.dst_port = H1_PORT

        elif host == "h8":
            self.src_ip = H8_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H8_PORT
            self.dst_port = H1_PORT

        elif host == "h9":
            self.src_ip = H9_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H9_PORT
            self.dst_port = H1_PORT

        elif host == "h10":
            self.src_ip = H10_ADDR
            self.dst_ip = H1_ADDR
            self.src_port = H10_PORT
            self.dst_port = H1_PORT
    ...
    def send_ack(self, ack_no):
        ...
        packet = scp.IP(src=self.src_ip, dst=self.dst_ip) / scp.TCP(
            sport=self.src_port, dport=self.dst_port, flags="A", ack=ack_no
        )
        scp.send(packet, verbose=0)
    ...
    def receive(self):
        ...
        # ack received
            ...
            self.xprint(
                cc.OKGREEN + "(received) ack ack=:%d" % (pkt[scp.TCP].ack - 1) + cc.ENDC
            )
    ...
    def write_logs_to_files(self):
        filename = "attack_log.txt" if self.log_attacker else ("%s_log.txt" % host)
        f = open(filename, "w")
