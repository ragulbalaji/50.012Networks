...
def draw_cwnd_plot(time_h1, cwnd_h1, time_h3, cwnd_h3, alg, delay):
    ...
    plt.title(
        "Cwnd vs. Time Graph\n{0} TCP Congestion Control Algorithm Delay={1}ms".format(
            alg.capitalize(), delay
        )
    )
...
def parse_iperf_data(alg, delay, host_addrs):
    ...
    data = dict(
        {
            "h1": {"Mbps": list(), "time": list()},
            "h2": {"Mbps": list(), "time": list()},
            "h3": {"Mbps": list(), "time": list()},
            "h4": {"Mbps": list(), "time": list()},
        }
    )
    ...
    with open("iperf_{0}_h1-h2_{1}ms.txt".format(alg, delay), "r") as fcsv:
        ...
        for row in r:
            if host_addrs["h1"] in row["src_addr"]:
                time = mktime(
                    datetime.strptime(str(row["time"]), "%Y%m%d%H%M%S").timetuple()
                )
    ...
    print(
        "h1: time={0}, bandwidth={1}".format(
            data["h1"]["time"].pop(), data["h1"]["Mbps"].pop()
        )
    )
    ...
    # Pop the last row because it is the average bandwidth of the session
    print(
        "h3: time={0}, bandwidth={1}".format(
            data["h3"]["time"].pop(), data["h3"]["Mbps"].pop()
        )
    )
...
def parse_tcpprobe_data(alg, delay, host_addrs):
    ...
    data = dict(
        {
            "h1": {"cwnd": list(), "time": list()},
            "h2": {"cwnd": list(), "time": list()},
            "h3": {"cwnd": list(), "time": list()},
            "h4": {"cwnd": list(), "time": list()},
        }
    )
    ...
    with open("tcpprobe_{0}_{1}ms.txt".format(alg, delay), "r") as fcsv:
        r = csv.DictReader(
            fcsv, delimiter=" ", fieldnames=tcpprobe_csv_header, restval=-1000
        )
        for row in r:
...
def tcp_tests(algs, delays, iperf_runtime, iperf_delayed_start):
    ...
    for alg in algs:
        ...
        for delay in delays:
            ...
            host_addrs = dict(
                {"h1": h1.IP(), "h2": h2.IP(), "h3": h3.IP(), "h4": h4.IP()}
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="TCP Congestion Control tests in a dumbbell topology."
    )
    parser.add_argument(
        "-a",
        "--algorithms",
    ...
    parser.add_argument(
        ...
        "--log-level",
        default="info",
        help="Verbosity level of the logger. Uses `info` by default.",
    )
    parser.add_argument(
        "-t",
        "--run-test",
        action="store_true",
        help="Run the dumbbell topology test.",
    )
    ...
    if args.run_test:
        ...
        tcp_tests(
            args.algorithms, args.delays, args.iperf_runtime, args.iperf_delayed_start
        )
